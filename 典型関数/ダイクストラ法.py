...
for i in range(M):
    flg, *aa = MI()
    #flg == 0だったらa[0]←→a[1]までの最小コストを出力
    if flg == 0:
        dist,v= Dijkstra(G,aa[0]-1)
        if dist[aa[1]-1] == inf:
            #dist[aa[1]-1] == inf　つまり、　a[0]→a[1]に行くことができないなら　-1　を出力
            print(-1)
        else:
            #経路があるならそれを出力
            print(dist[aa[1]-1])

    #flg== 1だったら　新しい経路として、a[0]←→a[1]の経路をコストa[2]として追加
    else:
        if G[aa[0]-1][aa[1]-1] == 0:
            G[aa[0]-1][aa[1]-1] = aa[2]
            #↓と5行下のコードをコメントアウトしたら、a[0]→a[1]の一方通行になる
            G[aa[1]-1][aa[0]-1] = aa[2]
        else:
            G[aa[0]-1][aa[1]-1] = min(G[aa[0]-1][aa[1]-1],aa[2])
            #↓と5行上のコードをコメントアウトしたら、a[0]→a[1]の一方通行になる
